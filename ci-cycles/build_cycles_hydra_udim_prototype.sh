...
: "${APPLY_UDIM_PATCH:?APPLY_UDIM_PATCH is required}"
...
git -C "$BUILD_ROOT/cycles" apply --check \
  /usr/local/share/cycles-hydra-udim.patch
if [[ "$APPLY_UDIM_PATCH" == "1" ]]; then
  git -C "$BUILD_ROOT/cycles" apply \
    /usr/local/share/cycles-hydra-udim.patch
elif [[ "$APPLY_UDIM_PATCH" != "0" ]]; then
  echo "APPLY_UDIM_PATCH must be 0 or 1" >&2
  exit 2
fi
git -C "$BUILD_ROOT/cycles" diff --check
git -C "$BUILD_ROOT/cycles" diff > "$EVIDENCE_ROOT/cycles-source.patch"
sha256sum "$BUILD_ROOT/cycles/src/hydra/material.cpp" > "$EVIDENCE_ROOT/material.cpp.sha256"
