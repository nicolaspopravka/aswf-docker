...
def _SetupOpenGLContext(width=100, height=100):
    ...
    egl_library = ("/usr/lib64/libEGL.so.1"
                   if os.environ.get("EGL_PLATFORM") == "surfaceless"
                   else "libEGL.so.1")
    egl = ctypes.cdll.LoadLibrary(egl_library)
    ...
    for attempt in range(max_attempts):
        if attempt > 0:
            ...
            _Msg("EGL: retry %d/%d in %.1fs..." % (attempt + 1, max_attempts, wait))
            time.sleep(wait)

        dpy = egl.eglGetDisplay(EGL_DEFAULT_DISPLAY)
        if not dpy:
            _Err("EGL: display selection failed (0x%04x)" % egl.eglGetError())
            continue
        major, minor = ctypes.c_int(), ctypes.c_int()
        if not egl.eglInitialize(dpy, ctypes.byref(major), ctypes.byref(minor)):
            _Err("EGL: initialization failed (0x%04x)" % egl.eglGetError())
            continue
        _Msg("EGL %d.%d" % (major.value, minor.value))
        if not egl.eglBindAPI(EGL_OPENGL_API):
            _Err("EGL: OpenGL API binding failed (0x%04x)" % egl.eglGetError())
            continue
        if not egl.eglChooseConfig(dpy, attrs, ctypes.byref(config), 1, ctypes.byref(n)):
            _Err("EGL: config selection failed (0x%04x)" % egl.eglGetError())
            continue
        if n.value < 1:
            _Err("EGL: config selection returned no matches")
            continue
        break
    else:
        raise RuntimeError("EGL: setup failed after %d attempts" % max_attempts)
    ...
    if fp:
        ...
        if raw_version:
            version_str = raw_version.decode()
            _Msg("GL_VERSION: %s" % version_str)
        if raw_renderer:
            renderer_str = raw_renderer.decode()
            _Msg("GL_RENDERER: %s" % renderer_str)

    allow_sw = os.environ.get("PXR_EGL_ALLOW_SOFTWARE_GL", "").lower() in ("1", "true", "yes")
    if renderer_str is None:
        _Err("GL_RENDERER is null — no GL context or Mesa library missing")
        if not allow_sw:
            raise RuntimeError("EGL: GL_RENDERER is null (set PXR_EGL_ALLOW_SOFTWARE_GL=1 to override)")
    else:
        rlow = renderer_str.lower()
        if not allow_sw and ("mesa" in rlow or "llvmpipe" in rlow or "software" in rlow):
            raise RuntimeError(
                "EGL: software GL detected (%s); set PXR_EGL_ALLOW_SOFTWARE_GL=1 "
                "to override" % renderer_str
            )

    # Keep Python references to the EGL objects alive for the lifetime of the
    # render.  The caller only needs a current GL context; no Qt/PySide
    # application object is required for headless EGL rendering.
    return {
        "egl": egl,
        "display": dpy,
        "context": ctx,
...
def main() -> int:
    ...
    # Initialize FrameRecorder (API differs between USD versions)
    usdVersion = Usd.GetVersion()[:2]  # (major, minor)
    if usdVersion >= (0, 26):
        ...
        if rendererPluginId:
            frameRecorder.SetRendererPlugin(rendererPluginId)
    if args.rpPrimPath:
    ...
    if hasattr(frameRecorder, "SetPrimaryCameraPrimPath"):
